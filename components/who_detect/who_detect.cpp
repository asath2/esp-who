...
#include "who_detect.hpp"
#include "esp_timer.h"  // added for inference timing
...
void WhoDetect::task()
{
    ...
    while (true) {
        ...
        // ⏱️ Inference timing start
        int64_t t1 = esp_timer_get_time();

        auto &res = m_model->run(img);

        // ⏱️ Inference timing end
        int64_t t2 = esp_timer_get_time();
        int64_t inference_ms = (t2 - t1) / 1000;
        float fps = (inference_ms > 0) ? (1000.0f / inference_ms) : 0.0f;
        ESP_LOGI("INFERENCE", "Time: %lld ms | FPS: %.1f | Objects detected: %d",
                 inference_ms,
                 fps,
                 (int)res.size());
