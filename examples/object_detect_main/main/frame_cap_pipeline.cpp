...
static const char *TAG = "FrameCapPipeline";
...
WhoFrameCap *get_lcd_dvp_frame_cap_pipeline()
{
    ...
    ESP_LOGI(TAG, "Using ESP32-S3 LCD DVP pipeline with sensor vertical flip only");
    auto cam = new WhoS3Cam(PIXFORMAT_RGB565, frame_size, MODEL_TIME + 3, false, true);
...
WhoFrameCap *get_term_dvp_frame_cap_pipeline()
{
    ...
    framesize_t frame_size = get_cam_frame_size_from_lcd_resolution();
    ESP_LOGI(TAG, "Using ESP32-S3 terminal DVP pipeline with sensor vertical flip only");
    auto cam = new WhoS3Cam(PIXFORMAT_RGB565, frame_size, MODEL_TIME + 2, false, true);
    auto frame_cap = new WhoFrameCap();
    frame_cap->add_node<WhoFetchNode>("FrameCapFetch", cam);
    return frame_cap;
}
